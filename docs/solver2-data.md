...
Perfect-play minimax solutions for Texas 42, generated by the GPU solver in `scripts/solver2/`.

Data files are stored in `data/solver2/` (gitignored due to size).
...
### V (State Value)
- Range: [-42, +42] (even integers only in practice)
- Positive: Team 0 (players 0,2) is winning
