...
1. **Incomplete Config Capture**: URLs only stored `seed`, `actions`, `playerTypes`, `dealer`, and theme. They did NOT capture `layers`, making it impossible to replay games with special contracts (nello, plunge, splash) or behavior layers (tournament mode, oneHand, speed).
...
```
?s={seed}&p={players}&d={dealer}&t={theme}&v={colors}&h={scenario}&l={layers}&a={actions}
```

**Example**:
```
?s=abc&p=haaa&l=t,n,p&a=CAAS
```
Decodes to: seed=12345, tournament/nello/plunge layers, actions=['C','A','A','S']
...
4. **Compact URLs**: Short-code registries keep URLs reasonably sized. Example: `l=t,o` = tournament + oneHand.

5. **Zero Compatibility Code**: Per requirement, no versioning. All URLs generated by new system. Big-bang migration, roll-forward.

### Negative

1. **Existing URLs Invalid**: Old URLs missing `l` param will replay with base rules only. Acceptable trade-off for greenfield project.
...
3. **Short Code Registry Maintenance**: Adding new layers requires updating `LAYER_CODES` map. But layers change rarely.
...
```
?s=xc9&l=t,n,p,S&a=CAAAMNO
```
- seed=12345
- tournament + nello + plunge + splash layers
- actions=['C','A','A','A','M','N','O']

### Full config URL
```
?s=xc9&p=hhaa&d=0&t=dark&l=t,o,n&a=C
```
- seed=12345
- 2 humans, 2 AI
- dealer=0
- dark theme
- tournament + oneHand + nello layers
- actions=['C']
