...
Round r (1..R):
  ...
  2. RETRAIN jud_net on the CUMULATIVE corpus: base chunks (mixed regimes:
     ...
     recipe-fork winner). Save versioned champion/jud_net_v2_r{r}.pt.
...
# --------------------------------------------------------------------- #
LOOP = Path("scratch/jud-v1/loop_v2")
...
EPS_EXPLORE = 0.15  # self-play only; A/B grading stays greedy


class EpsilonJudPlay(JudPlay):
    """JudPlay with per-move ε-uniform exploration (v2 loop, #69).

    The v1 corpus was greedy at every seat: 4459/5929 deals carried exactly ONE
    distinct opening lead, so 6 of 7 serve-time children were off-distribution.
    ε-exploration is the behavior policy for SELF-PLAY corpus generation only —
    the single realized label then covers a widening set of action prefixes.
    """

    def __init__(self, model, device="cpu", eps=EPS_EXPLORE, seed=0):
        super().__init__(model, device)
        self.eps = eps
        self._rng = __import__("random").Random(seed)

    def choose(self, states, bid_values, marks=None, marks_to_win=7):
        from forge.zeb.game import legal_actions as _legal
        picks = super().choose(states, bid_values, marks, marks_to_win)
        return [
            self._rng.choice(list(_legal(s))) if self._rng.random() < self.eps else p
            for s, p in zip(states, picks)
        ]


def _jud_team(head_path, explore: bool = False):
    head = JN.load_jud_net(head_path, JUD_DEVICE)
    play = (EpsilonJudPlay(head, JUD_DEVICE) if explore
            else JudPlay(head, JUD_DEVICE))
    return ValueBidder(head, MarksToSeven()), play
...
def self_play(head_path, r: int) -> Path:
    ...
    if out.exists():
        ...
        return out
    bid, play = _jud_team(head_path, explore=True)
    res = run_match(
...
def train_round(r: int, device: str) -> Path:
    head = Path(f"champion/jud_net_v2_r{r}.pt")
...
def main() -> int:
    ...
    head0 = Path("champion/jud_net_v2_r0.pt")
    ...
    for r in range(1, rounds + 1):
        ...
        head_r = Path(f"champion/jud_net_v2_r{r}.pt")
