---
name: texas-42
description: Wiki-first Texas 42 repo navigation and game development. Use for architecture, Burl/Gus/LEM/forge context, layer system implementation, testing patterns, URL debugging, game rule clarifications, special contracts (nello/splash/plunge/sevens), or debugging game states; read wiki/AGENTS.md and relevant wiki pages before code.
---
...
The project wiki is the authority for Texas 42 project context. This skill is
a routing aid, not a parallel source of truth.

Before answering architecture, history, Burl/Gus/LEM/forge, or "what's next"
questions, read `wiki/AGENTS.md` and then query the relevant wiki pages
(`wiki/index.md`, entity pages, topic pages, experiment pages, decisions,
playbooks, and log entries) before diving into code.

Use the quick references below only as secondary navigation and stale-check
targets. If this skill disagrees with the wiki, trust the wiki and update this
skill or file a bead to correct it.
...
- `src/game/layers/{nello,splash,plunge,sevens}.ts` - Special contracts (game rule variants)
- `src/game/layers/{tournament,oneHand,hints}.ts` - Game mode layers
- `src/game/layers/consensus.ts` - Gates progress with human acknowledgment (agree-trick/agree-score)
- `src/game/layers/speed.ts` - Auto-executes forced moves (single legal actions)
