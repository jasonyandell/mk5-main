...
import type { GameLayer } from '../../../game/layers/types';
...
describe('Threaded Rules in GameEngine', () => {
  it('should use composed rules for bid validation in getBiddingActions', () => {
    // Create a restrictive layer that blocks all NELLO bids through rules
    const restrictiveLayer: GameLayer = {
      name: 'restrictive',
      rules: {
    ...
    // (base layer's isValidBid returns false for NELLO)
    const baseOnlyRules = composeRules([baseLayer]);
    ...
    expect(nelloBidsBaseOnly.length).toBe(0); // Base layer doesn't validate NELLO

    // With permissive rules (no validation): NELLO bids should be generated
    // (permissive layer allows all bids that base layer allows, plus NELLO)
    const permissiveLayer: GameLayer = {
      name: 'permissive',
      rules: {
        isValidBid: (_state: GameState, bid: Bid, _playerHand: unknown, prev: boolean) => {
          // Allow NELLO bids
    ...
    const permissiveRules = composeRules([baseLayer, permissiveLayer]);
    ...
    expect(nelloBidsPermissive.length).toBeGreaterThan(0); // Permissive layer allows NELLO

    // With restrictive layer: rules should filter out NELLO bids during validation
    const restrictiveRules = composeRules([baseLayer, permissiveLayer, restrictiveLayer]);
    ...
    expect(nelloBidsWithRestriction.length).toBe(0); // Restrictive layer blocks NELLO through rules
  });

  it('should use composed rules for play validation in getPlayingActions', () => {
    // Create a test layer that restricts plays to only the first domino
    const restrictiveLayer: GameLayer = {
    ...
    // Without layer, all dominoes should be playable (first play of trick)
    const actionsWithoutLayer = getValidActions(state);
    expect(actionsWithoutLayer.length).toBe(3);

    // With restrictive layer, should only return first domino
