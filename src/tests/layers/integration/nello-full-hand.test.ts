...
describe('Nello Full Hand Integration', () => {
  const layers = [baseLayer, nelloLayer];
  ...
  async function playNelloHand(
    ...
    // Select nello trump
    const nelloTransition = getNextStates(state, layers, rules).find(t =>
  ...
  describe('Successful Nello', () => {
    ...
    it('should treat doubles as suit 7 (own suit)', async () => {
      ...
      const nelloTransition = getNextStates(testState, layers, rules).find(t =>
      ...
      const doublePlay = getNextStates(testState, layers, rules).find(t =>
  ...
  describe('Player Rotation', () => {
    it('should skip partner in turn order', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const playTransitions = getNextStates(testState, layers, rules).filter(t => t.action.type === 'play');
