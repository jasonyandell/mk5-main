...
import { ACES, DEUCES, TRES, FIVES, NO_LEAD_SUIT, DOUBLES_AS_TRUMP } from '../../game/types';
...
describe('Renege Detection and Prevention', () => {
  describe('Must Follow Suit When Able', () => {
    ...
    it('correctly identifies trump vs non-trump suits', () => {
      ...
      expect(getLedSuitBase(state, trumpDomino)).toBe(DOUBLES_AS_TRUMP); // Absorbed suit
      expect(getLedSuitBase(state, nonTrumpDomino)).not.toBe(DOUBLES_AS_TRUMP); // Not absorbed
    });
  });
