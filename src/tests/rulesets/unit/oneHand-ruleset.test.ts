/**
 * Tests for OneHand RuleSet
 *
 * Verifies that oneHandRuleSet properly overrides phase transition logic
 * to create terminal state for single-hand game mode.
 */

import { describe, it, expect } from 'vitest';
import { oneHandRuleSet } from '../../../game/rulesets/oneHand';
...
describe('OneHand RuleSet', () => {
  ...
  describe('ruleset structure', () => {
    ...
    it('should only override getPhaseAfterHandComplete', () => {
      expect(oneHandRuleSet.rules).toBeDefined();
      expect(Object.keys(oneHandRuleSet.rules!)).toEqual(['getPhaseAfterHandComplete']);
    });

    it('should not provide getValidActions override', () => {
      expect(oneHandRuleSet.getValidActions).toBeUndefined();
    });
  });
});
