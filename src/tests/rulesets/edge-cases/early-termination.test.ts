...
/**
 * Early Termination - Testing that hands end correctly when outcome is determined:
 * - Nello: Ends when bidder wins any trick
 * - Plunge: Ends when opponents win any trick
 * - Splash: Ends when opponents win any trick
 * - Sevens: Ends when opponents win any trick
 * - All contracts: Correct decidedAtTrick value, proper phase transition, remaining dominoes in hands
 */
describe('Early Termination', () => {
  ...
  describe('Nello Early Termination', () => {
    it('should terminate on first trick won by bidder', () => {
      const state = GameTestHelper.createTestState({
        ...
        tricks: [
          {
            plays: [
              { player: 0, domino: { id: 't1-0', high: SIXES, low: BLANKS } }, // Bidder plays high
              { player: 1, domino: { id: 't1-1', high: ACES, low: BLANKS } },
              { player: 3, domino: { id: 't1-3', high: DEUCES, low: BLANKS } }
            ],
            winner: 0, // Bidder won (failure)
            points: 0,
            ledSuit: SIXES
          }
        ]
      ...
      // Check hand outcome - should be determined
      const outcome = nelloRules.checkHandOutcome(state);
      expect(outcome).not.toBeNull();
      expect(outcome?.isDetermined).toBe(true);
      expect(outcome?.decidedAtTrick).toBe(1); // First trick
      expect(outcome?.reason).toContain('Bidding team won trick 1');
    });

    it('should not terminate when bidder loses all tricks so far', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'nello' },
        winningBidder: 0,
        ...
        tricks: [
          {
            plays: [
              { player: 0, domino: { id: 't1-0', high: ACES, low: BLANKS } }, // Bidder plays low
              { player: 1, domino: { id: 't1-1', high: SIXES, low: BLANKS } }, // Opponent wins
              { player: 3, domino: { id: 't1-3', high: DEUCES, low: BLANKS } }
            ],
            winner: 1, // Opponent won (success so far)
            points: 0,
            ledSuit: ACES
          },
          {
            plays: [
              ...
              { player: 3, domino: { id: 't2-3', high: FIVES, low: BLANKS } }, // Opponent wins
              { player: 0, domino: { id: 't2-0', high: ACES, low: BLANKS } }
            ],
            winner: 3, // Opponent won
            points: 0,
            ledSuit: DEUCES
          }
        ]
      ...
      // Check hand outcome - should NOT be determined yet
      const outcome = nelloRules.checkHandOutcome(state);
      expect(outcome).toBeNull(); // Continue playing
    });

    it('should terminate on 3rd trick won by bidder', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'nello' },
        winningBidder: 0,
        ...
        tricks: [
          {
            plays: [
              ...
              { player: 1, domino: { id: 't1-1', high: SIXES, low: BLANKS } },
              { player: 3, domino: { id: 't1-3', high: DEUCES, low: BLANKS } }
            ],
          ...
          },
          {
            plays: [
              { player: 1, domino: { id: 't2-1', high: DEUCES, low: BLANKS } },
              { player: 3, domino: { id: 't2-3', high: FIVES, low: BLANKS } },
              { player: 0, domino: { id: 't2-0', high: ACES, low: BLANKS } }
            ],
            winner: 3, // Opponent won
            points: 0,
          ...
          {
            plays: [
              { player: 3, domino: { id: 't3-3', high: TRES, low: BLANKS } },
              { player: 0, domino: { id: 't3-0', high: SIXES, low: BLANKS } }, // Bidder wins
              { player: 1, domino: { id: 't3-1', high: ACES, low: BLANKS } }
            ],
            winner: 0, // Bidder won on trick 3
            points: 0,
            ledSuit: TRES
          }
        ]
      ...
      expect(outcome).not.toBeNull();
      expect(outcome?.isDetermined).toBe(true);
      expect(outcome?.decidedAtTrick).toBe(3); // Third trick
    });
  });

  describe('Plunge Early Termination', () => {
    it('should terminate on first trick won by opponents', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        ...
        tricks: [
          {
            plays: [
              { player: 2, domino: { id: 't1-2', high: ACES, low: BLANKS } }, // Partner leads
              { player: 3, domino: { id: 't1-3', high: SIXES, low: BLANKS } }, // Opponent plays high non-trump
              { player: 0, domino: { id: 't1-0', high: DEUCES, low: BLANKS } },
              { player: 1, domino: { id: 't1-1', high: TRES, low: BLANKS } }
            ],
    ...
    it('should not terminate when bidding team wins all tricks so far', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'suit', suit: ACES },
        winningBidder: 0,
        ...
        tricks: [
          {
            plays: [
              { player: 2, domino: { id: 't1-2', high: SIXES, low: BLANKS } },
              { player: 3, domino: { id: 't1-3', high: DEUCES, low: BLANKS } },
              { player: 0, domino: { id: 't1-0', high: ACES, low: ACES } }, // Bidder plays trump
              { player: 1, domino: { id: 't1-1', high: TRES, low: BLANKS } }
            ],
            winner: 0, // Bidder's team won
            points: 10,
            ledSuit: SIXES
          },
          {
            plays: [
              { player: 0, domino: { id: 't2-0', high: FIVES, low: BLANKS } },
              { player: 1, domino: { id: 't2-1', high: ACES, low: DEUCES } },
              { player: 2, domino: { id: 't2-2', high: SIXES, low: FIVES } }, // Partner wins
              { player: 3, domino: { id: 't2-3', high: TRES, low: BLANKS } }
            ],
      ...
      const outcome = plungeRules.checkHandOutcome(state);
      expect(outcome).toBeNull(); // Not determined yet - still winning
    });

    it('should terminate on 5th trick won by opponents', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'doubles' },
        winningBidder: 0,
        ...
        currentBid: { type: 'plunge', value: 4, player: 0 },
        tricks: [
          { plays: [], winner: 0, points: 0, ledSuit: ACES }, // Bidder won
          { plays: [], winner: 2, points: 0, ledSuit: DEUCES }, // Partner won
          { plays: [], winner: 0, points: 5, ledSuit: TRES }, // Bidder won
          { plays: [], winner: 2, points: 10, ledSuit: FOURS }, // Partner won
          {
            plays: [
              { player: 2, domino: { id: 't5-2', high: FIVES, low: BLANKS } },
              { player: 3, domino: { id: 't5-3', high: FIVES, low: FIVES } }, // Opponent plays double
              { player: 0, domino: { id: 't5-0', high: ACES, low: BLANKS } },
              { player: 1, domino: { id: 't5-1', high: DEUCES, low: BLANKS } }
            ],
            winner: 3, // Opponent won on trick 5
            points: 10,
            ledSuit: FIVES
          }
        ]
      ...
      expect(outcome).not.toBeNull();
      expect(outcome?.isDetermined).toBe(true);
      expect(outcome?.decidedAtTrick).toBe(5);
    });
  });

  describe('Splash Early Termination', () => {
    it('should terminate on first trick won by opponents', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        ...
        tricks: [
          {
            plays: [
              { player: 3, domino: { id: 't1-3', high: ACES, low: BLANKS } }, // Partner leads
              { player: 0, domino: { id: 't1-0', high: DEUCES, low: DEUCES } }, // Opponent plays trump
              { player: 1, domino: { id: 't1-1', high: ACES, low: ACES } },
              { player: 2, domino: { id: 't1-2', high: TRES, low: BLANKS } }
            ],
            winner: 0, // Opponent won (splash failed)
            points: 10,
            ledSuit: ACES
          }
        ]
    ...
    it('should not terminate when bidding team wins first 3 tricks', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'no-trump' },
        winningBidder: 1,
        currentPlayer: 3,
        currentBid: { type: 'splash', value: 3, player: 1 },
        tricks: [
          { plays: [], winner: 3, points: 5, ledSuit: ACES },  // Partner won
          { plays: [], winner: 1, points: 10, ledSuit: DEUCES }, // Bidder won
          { plays: [], winner: 3, points: 0, ledSuit: TRES }   // Partner won
        ]
      });

      const outcome = splashRules.checkHandOutcome(state);
      expect(outcome).toBeNull(); // Not determined - still winning all tricks
    });

    it('should terminate on 2nd trick won by opponents', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'suit', suit: SIXES },
        winningBidder: 2, // Player 2, team 0
        currentPlayer: 0,
        currentBid: { type: 'splash', value: 2, player: 2 },
        tricks: [
          {
            plays: [
              { player: 0, domino: { id: 't1-0', high: ACES, low: BLANKS } },
              { player: 1, domino: { id: 't1-1', high: DEUCES, low: BLANKS } },
              { player: 2, domino: { id: 't1-2', high: SIXES, low: BLANKS } }, // Bidder plays trump
              { player: 3, domino: { id: 't1-3', high: TRES, low: BLANKS } }
            ],
            winner: 2, // Bidder won (team 0)
            points: 0,
            ledSuit: ACES
          },
          {
            plays: [
              { player: 2, domino: { id: 't2-2', high: FOURS, low: BLANKS } },
              { player: 3, domino: { id: 't2-3', high: SIXES, low: FIVES } },
              { player: 0, domino: { id: 't2-0', high: FOURS, low: FOURS } }, // Partner plays double (team 0)
              { player: 1, domino: { id: 't2-1', high: SIXES, low: SIXES } } // Opponent plays trump double - WINS (team 1)
            ],
            winner: 1, // Opponent won on trick 2 (team 1)
            points: 10,
            ledSuit: FOURS
          }
        ]
  ...
  describe('Sevens Early Termination', () => {
    it('should terminate on first trick won by opponents', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'sevens' },
        ...
        tricks: [
          {
            plays: [
              { player: 0, domino: { id: 't1-0', high: ACES, low: BLANKS } }, // 1 pip (distance 6)
              { player: 1, domino: { id: 't1-1', high: TRES, low: FOURS } }, // 7 pips (distance 0) - WINS
              { player: 2, domino: { id: 't1-2', high: SIXES, low: BLANKS } }, // 6 pips (distance 1)
              { player: 3, domino: { id: 't1-3', high: DEUCES, low: DEUCES } } // 4 pips (distance 3)
            ],
            winner: 1, // Opponent won with closest to 7
            points: 5,
            ledSuit: ACES
          }
        ]
    ...
    it('should not terminate when bidding team wins all tricks so far', () => {
      const state = GameTestHelper.createTestState({
        ...
        tricks: [
          {
            plays: [
              { player: 0, domino: { id: 't1-0', high: TRES, low: FOURS } }, // 7 pips - WINS
              { player: 1, domino: { id: 't1-1', high: ACES, low: BLANKS } },
              { player: 2, domino: { id: 't1-2', high: SIXES, low: BLANKS } },
              { player: 3, domino: { id: 't1-3', high: DEUCES, low: DEUCES } }
            ],
            winner: 0, // Bidder won
            points: 5,
            ledSuit: TRES
          ...
          {
            plays: [
              { player: 0, domino: { id: 't2-0', high: ACES, low: ACES } },
              { player: 1, domino: { id: 't2-1', high: FOURS, low: TRES } }, // 7 pips
              { player: 2, domino: { id: 't2-2', high: SIXES, low: ACES } }, // 7 pips - partner wins (first)
              { player: 3, domino: { id: 't2-3', high: FIVES, low: BLANKS } }
            ],
            winner: 2, // Partner won (tie goes to first)
            points: 5,
            ledSuit: ACES
          }
        ]
      ...
      const outcome = sevensRules.checkHandOutcome(state);
      expect(outcome).toBeNull(); // Not determined - team still winning
    });

    it('should terminate on 4th trick won by opponents', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'sevens' },
        winningBidder: 3,
        ...
        tricks: [
          { plays: [], winner: 3, points: 0, ledSuit: BLANKS }, // Bidder won
          { plays: [], winner: 1, points: 5, ledSuit: ACES },   // Partner won
          { plays: [], winner: 3, points: 10, ledSuit: DEUCES }, // Bidder won
          {
            plays: [
              { player: 3, domino: { id: 't4-3', high: ACES, low: BLANKS } }, // 1 pip
              { player: 0, domino: { id: 't4-0', high: FIVES, low: DEUCES } }, // 7 pips - WINS
              { player: 1, domino: { id: 't4-1', high: SIXES, low: SIXES } },
              { player: 2, domino: { id: 't4-2', high: TRES, low: TRES } }
            ],
            winner: 0, // Opponent won on trick 4
            points: 5,
            ledSuit: ACES
          }
        ]
  ...
  describe('Remaining Dominoes Stay in Hands', () => {
    it('should keep unplayed dominoes in player hands after early termination', () => {
      const state = GameTestHelper.createTestState({
        phase: 'playing',
        trump: { type: 'nello' },
        winningBidder: 0,
