...
export interface GameRules {
   ...
   * RuleSets can override for special bid rules
   */
...
/**
 * GameRuleSet interface - Composable rule set that can override rules and/or transform actions.
 *
 * RuleSets have two orthogonal composition surfaces:
 * 1. Action generation (getValidActions) - what's possible
 * 2. Rule algorithms (rules) - how things execute
 *
 * RuleSets compose via reduce, with later rule sets overriding earlier ones.
 */
export interface GameRuleSet {
  name: string;

  /**
   * Transform action generation (works alongside action transformer system).
   *
   * Pattern: Filter, annotate, or add actions.
   *
   * Example: Tournament rule set filters special contract bids
   * Example: Nello rule set adds nello trump selection option
   *
   * @param state Current game state
   * @param prev Actions from previous rule sets (base or other rule sets)
   ...
   *
   * Each rule method receives the previous rule set's result as its last parameter.
   * Return prev to pass through, or return new value to override.
   *
   * Example: Nello rule set returns 3 for isTrickComplete
   * Example: Plunge rule set returns partner index for getTrumpSelector
   *
