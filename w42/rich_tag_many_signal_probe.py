"""W42 rich-tag many-signal probe.

This script is w42-owned research code. It keeps the raw/v0 split, core encoder
shape, metrics, and raw checkpoint comparison intact, then adds a richer
chapter-bucket feature surface derived from public v0 global/action-local tags.
The many-signal layer is deliberately cheap and public-state safe: it projects
aggregates, interactions, and bucket proxies from existing detector tensors
without touching Gus core model/training paths or using oracle values as inputs.
"""
...
def _tag_index(tags: list[Any], name: str) -> int:
    for tag in tags:
        if tag.name == name:
            return tag.idx
    raise KeyError(name)
...
def _group_indices(tags: list[Any], group: str) -> list[int]:
    return group_slices(tags)[group]


ACTION_GROUP_IDXS = {group: idxs for group, idxs in group_slices(ACTION_TAGS).items()}
GLOBAL_GROUP_IDXS = {group: idxs for group, idxs in group_slices(GLOBAL_TAGS).items()}

ACTION_IDX = {tag.name: tag.idx for tag in ACTION_TAGS}
GLOBAL_IDX = {tag.name: tag.idx for tag in GLOBAL_TAGS}

RICH_GLOBAL_SIGNAL_NAMES = [
    "legal_action_frac",
    "must_follow_or_restricted",
    "decision_fraction",
    "trick_index_fraction",
    "trick_position_fraction",
    "current_hand_frac",
    "hand_trump_frac",
    ...
    "legal_point_dump_any",
    "legal_beats_current_any",
    ...
    "count_budget_proxy",
    "late_hand_proxy",
    "void_attention_proxy",
    "unseen_count_mean",
    "visible_pip_mean",
    "own_pip_mean",
    "hand_low_high_spread",
    "legal_count_per_legal_action",
    "action_signal_density",
]
...
RICH_ACTION_SIGNAL_NAMES = [
    ...
    "candidate_called_rank_frac",
    "candidate_off_non_double",
    "candidate_follows_led",
    ...
    "candidate_identity_density",
    "candidate_tactical_density",
    ...
    "candidate_84_preservation_proxy",
    "candidate_scoring_pressure_proxy",
    "candidate_no_trump_double_proxy",
    "candidate_bidding_risk_proxy",
    "global_must_follow_context",
    "global_late_hand_context",
    "global_current_trick_count_context",
    "global_unknown_count_context",
]


def _select(values: Tensor, names: list[str], index: dict[str, int]) -> Tensor:
    return values[..., [index[name] for name in names]]


def _any_group(action_values: Tensor, group: str, legal: Tensor) -> Tensor:
    vals = action_values[:, :, ACTION_GROUP_IDXS[group]].abs().sum(dim=-1)
    return ((vals > 1e-9) & legal).float().max(dim=1).values


def _mean_group(action_values: Tensor, group: str, legal: Tensor) -> Tensor:
    vals = action_values[:, :, ACTION_GROUP_IDXS[group]].abs().sum(dim=-1)
    denom = legal.float().sum(dim=1).clamp_min(1.0)
    return (vals * legal.float()).sum(dim=1) / denom


def _rich_global_features(batch: dict[str, Tensor]) -> Tensor:
    g = batch["strategy_features"].float()
    a = batch["strategy_action_features"].float()
    legal = batch["legal_mask"].bool()
    legal_f = legal.float()
    denom = legal_f.sum(dim=1).clamp_min(1.0)

    def gv(name: str) -> Tensor:
        return g[:, GLOBAL_IDX[name]]

    def av(name: str) -> Tensor:
        return a[:, :, ACTION_IDX[name]]

    legal_count_points = (av("count_points_frac") * legal_f).sum(dim=1) / denom
    legal_action_density = (a.abs().sum(dim=-1) * legal_f).sum(dim=1) / denom
    unseen_count_mean = g[:, GLOBAL_GROUP_IDXS["unseen_count_by_pip"]].mean(dim=1)
    visible_pip_mean = g[:, GLOBAL_GROUP_IDXS["visible_pip_coverage"]].mean(dim=1)
    own_pip = g[:, GLOBAL_GROUP_IDXS["own_pip_coverage"]]
    own_pip_mean = own_pip.mean(dim=1)
    hand_low_high_spread = own_pip.max(dim=1).values - own_pip.min(dim=1).values

    cols = [
        gv("legal_action_frac"),
        gv("must_follow_or_restricted"),
        gv("decision_fraction"),
        gv("trick_index_fraction"),
        gv("trick_position_fraction"),
        gv("current_hand_frac"),
        gv("hand_trump_frac"),
        gv("hand_count_point_frac"),
        gv("hand_off_count_point_frac"),
        gv("legal_trump_frac"),
        gv("legal_count_point_frac"),
        gv("unknown_count_point_frac"),
        gv("current_trick_count_point_frac"),
        gv("current_trick_has_trump"),
        gv("partner_currently_winning"),
        gv("opponent_currently_winning"),
        gv("is_lead_position"),
        gv("is_last_to_play"),
        _any_group(a, "donation_window", legal),
        _any_group(a, "count_pressure", legal),
        _any_group(a, "double_protection", legal),
        _mean_group(a, "suit_pressure", legal),
        ((av("trump") > 0) & legal).float().max(dim=1).values,
        ((av("off_non_double") > 0) & legal).float().max(dim=1).values,
        ((av("point_dump") > 0) & legal).float().max(dim=1).values,
        ((av("beats_current") > 0) & legal).float().max(dim=1).values,
        ((av("count_donation_to_partner") > 0) & legal).float().max(dim=1).values,
        ((av("count_donation_to_opponent") > 0) & legal).float().max(dim=1).values,
        _any_group(a, "count_pressure", legal) * gv("opponent_currently_winning"),
        _any_group(a, "donation_window", legal) * gv("partner_currently_winning"),
        _any_group(a, "double_protection", legal) * gv("hand_off_count_point_frac"),
        _mean_group(a, "suit_pressure", legal) * gv("must_follow_or_restricted"),
        gv("is_lead_position") * legal_count_points,
        gv("is_last_to_play") * gv("current_trick_count_point_frac"),
        gv("hand_off_count_point_frac") * (1.0 - _any_group(a, "double_protection", legal)),
        gv("hand_trump_frac") * _mean_group(a, "suit_pressure", legal),
        gv("opponent_currently_winning") * _any_group(a, "count_pressure", legal),
        gv("partner_currently_winning") * _any_group(a, "donation_window", legal),
        gv("hand_off_count_point_frac") * gv("legal_count_point_frac"),
        gv("unknown_count_point_frac") * gv("legal_count_point_frac"),
        gv("decision_fraction") * gv("current_hand_frac"),
        gv("void_mean_all") * gv("must_follow_or_restricted"),
        unseen_count_mean,
        visible_pip_mean,
        own_pip_mean,
        hand_low_high_spread,
        legal_count_points,
        legal_action_density / max(float(len(ACTION_TAGS)), 1.0),
    ]
    return torch.stack(cols, dim=-1)


def _rich_action_features(batch: dict[str, Tensor]) -> Tensor:
    g = batch["strategy_features"].float()
    a = batch["strategy_action_features"].float()

    def gv(name: str) -> Tensor:
        return g[:, GLOBAL_IDX[name]].unsqueeze(1).expand(-1, a.shape[1])

    def av(name: str) -> Tensor:
        return a[:, :, ACTION_IDX[name]]

    identity_density = a[:, :, ACTION_GROUP_IDXS["identity"]].abs().mean(dim=-1)
    tactical_density = a[:, :, ACTION_GROUP_IDXS["trick_relation"]].abs().mean(dim=-1)
    count_pressure = a[:, :, ACTION_GROUP_IDXS["count_pressure"]].abs().sum(dim=-1)
    suit_pressure = a[:, :, ACTION_GROUP_IDXS["suit_pressure"]].abs().sum(dim=-1)
    double_protection = a[:, :, ACTION_GROUP_IDXS["double_protection"]].abs().sum(dim=-1)
    donation_window = a[:, :, ACTION_GROUP_IDXS["donation_window"]].abs().sum(dim=-1)
    late_hand = gv("decision_fraction") * gv("current_hand_frac")
    ...
    cols = [
        ...
        av("point_dump"),
        av("live_suit_frac"),
        av("live_higher_suit_frac"),
        av("live_count_frac"),
        av("live_higher_count_frac"),
        av("pip_count_risk"),
        av("protected_by_my_double"),
        av("protected_by_high_double"),
        av("count_donation_to_partner"),
        av("count_donation_to_opponent"),
        count_pressure,
        suit_pressure,
        double_protection,
        donation_window,
        identity_density,
        tactical_density,
        av("count_points_frac") * av("beats_current"),
        av("count_points_frac") * gv("partner_currently_winning"),
        av("count_points_frac") * gv("opponent_currently_winning"),
        av("off_non_double") * double_protection.clamp(max=1.0),
        av("trump") * av("live_higher_suit_frac"),
        av("is_ten_count") * gv("is_last_to_play"),
        av("count_donation_to_partner") * gv("partner_currently_winning"),
        av("point_dump") * gv("opponent_currently_winning"),
        av("count_points_frac") * gv("opponent_currently_winning") * av("beats_current"),
        av("count_donation_to_partner") * gv("partner_currently_winning") * av("legal"),
        av("count_donation_to_opponent") * gv("opponent_currently_winning") * av("legal"),
        av("off_non_double") * (1.0 + double_protection.clamp(max=1.0)),
        av("trump") * (1.0 + suit_pressure.clamp(max=1.0)),
        av("beats_current") * av("live_higher_suit_frac").neg().add(1.0),
        av("double") * late_hand,
        av("count_points_frac") * gv("current_trick_count_point_frac"),
        av("double") * gv("current_trick_has_trump").neg().add(1.0),
        av("off_non_double") * gv("hand_off_count_point_frac"),
        gv("must_follow_or_restricted"),
        late_hand,
        gv("current_trick_count_point_frac"),
        gv("unknown_count_point_frac"),
...
def _rich_bucket_masks(batch: dict[str, Tensor]) -> dict[str, Tensor]:
    g = batch["strategy_features"].float()
    a = batch["strategy_action_features"].float()
    legal = batch["legal_mask"].bool()
    ...
    def av_any(name: str) -> Tensor:
        return ((a[:, :, ACTION_IDX[name]].abs() > 1e-9) & legal).any(dim=1)

    masks = {
        "bidding_risk": (
            (gv("hand_off_count_point_frac") > 0)
            | (gv("legal_count_point_frac") > 0)
            | (gv("unknown_count_point_frac") > 0.25)
        ),
        "count_donation": av_any("count_donation_to_partner") | av_any("point_dump") | av_any("is_ten_count"),
        "trump_pressure": av_any("trump") | (gv("current_trick_has_trump") > 0) | (gv("hand_trump_frac") > 0),
        "off_protection": av_any("off_non_double") | av_any("protected_by_my_double") | av_any("protected_by_high_double"),
        "pounce_window": av_any("count_donation_to_opponent") | ((gv("opponent_currently_winning") > 0) & av_any("count_points_frac")),
        "eighty_four_preservation": av_any("double") & (gv("decision_fraction") > 0.5),
        "walker_endgame": av_any("beats_current") & (gv("decision_fraction") > 0.6),
        "no_trump_doubles": av_any("double"),
        "scoring_pressure": (gv("current_trick_count_point_frac") > 0) | av_any("count_points_frac"),
    }
    return {name: mask.bool() for name, mask in masks.items()}


class RawPlusRichTagsModel(nn.Module):
    """Tiny policy scorer over public state, v0 tags, and rich public proxies."""
    ...
    def forward(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        state = self.norm(
            ...
            + self.rich_global_proj(_rich_global_features(batch))
        )
        ...
        actions = actions + self.rich_action_proj(_rich_action_features(batch))
...
def _metric_row(regret: Tensor, action: Tensor, oracle_action: Tensor) -> dict[str, float]:
    ...
    return {
        ...
        "tail_regret_rate_ge_5": float((regret >= 5.0).float().mean().item()) if total else 0.0,
        "max_regret": float(regret.max().item()) if total else 0.0,
        "n": float(total),
    }


def _empty_acc() -> dict[str, float]:
    return {"regret_sum": 0.0, "hits": 0.0, "near": 0.0, "tail_ge_5": 0.0, "max_regret": 0.0, "n": 0.0}
...
def _add_acc(acc: dict[str, float], regret: Tensor, action: Tensor, oracle_action: Tensor, mask: Tensor) -> None:
    ...
    acc["tail_ge_5"] += float((r >= 5.0).sum().item())
    acc["max_regret"] = max(acc["max_regret"], float(r.max().item()))
    acc["n"] += float(r.numel())
...
def _finish_acc(acc: dict[str, float]) -> dict[str, float]:
    ...
    return {
        ...
        "tail_regret_rate_ge_5": acc["tail_ge_5"] / n,
        "max_regret": acc["max_regret"],
        "n": acc["n"],
    }
...
def _active_many_signal_masks(batch: dict[str, Tensor]) -> dict[str, Tensor]:
    masks = _active_group_masks(batch)
    for bucket, mask in _rich_bucket_masks(batch).items():
        masks[f"rich:{bucket}"] = mask
    return masks
...
def evaluate_model_with_buckets(
    ...
    for batch in loader:
        ...
        active_masks = _active_many_signal_masks(batch)
...
def _comparison_metrics(
    ...
    rich_final: dict[str, float],
    rich_best: dict[str, Any],
    v0_best: dict[str, Any],
    v0_final: dict[str, Any],
    eq_metrics: dict[str, float],
) -> dict[str, Any]:
    return {
        "rich_best_minus_raw_best_mean_regret": rich_best["mean_regret"] - raw_best["mean_regret"],
        "rich_best_minus_raw_best_match_rate": rich_best["match_rate"] - raw_best["match_rate"],
        "rich_best_minus_raw_best_near_tie": rich_best["near_tie_rate_regret_lt_0_5"]
        - raw_best["near_tie_rate_regret_lt_0_5"],
        "rich_final_minus_raw_final_checkpoint_mean_regret": rich_final["mean_regret"]
        - raw_final["mean_regret"],
        "rich_best_minus_v0_best_mean_regret": rich_best["mean_regret"] - v0_best["mean_regret"],
        "rich_best_minus_v0_best_match_rate": rich_best["match_rate"] - v0_best["match_rate"],
        "rich_final_minus_v0_final_mean_regret": rich_final["mean_regret"] - v0_final["mean_regret"],
        "rich_best_minus_e_q_n_mean_regret": rich_best["mean_regret"] - eq_metrics["mean_regret"],
        "rich_best_minus_raw_final_checkpoint_tail_ge_5": rich_best["tail_regret_rate_ge_5"]
        - raw_final["tail_regret_rate_ge_5"],
        "rich_final_tail_regret_rate_ge_5": rich_final["tail_regret_rate_ge_5"],
        "rich_best_tail_regret_rate_ge_5": rich_best["tail_regret_rate_ge_5"],
    }
...
def _bucket_comparison(
    ...
    for bucket in sorted(set(raw_buckets) & set(tagged_buckets)):
        ...
        out[bucket] = {
            ...
            "raw_tail_ge_5": raw["tail_regret_rate_ge_5"],
            "tagged_tail_ge_5": tagged["tail_regret_rate_ge_5"],
            "delta_tail_ge_5": tagged["tail_regret_rate_ge_5"] - raw["tail_regret_rate_ge_5"],
            "n": tagged["n"],
        }
    return out
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--raw-metrics-path", default="w42/raw_public_state_baseline/metrics.json")
    parser.add_argument("--raw-checkpoint", default="w42/raw_public_state_baseline/model.pt")
    parser.add_argument("--v0-metrics-path", default="w42/v0_strategy_tags_baseline/metrics.json")
    parser.add_argument("--output-dir", default="w42/rich_tag_many_signal_probe")
...
def main() -> int:
    ...
    v0_metrics_file = ROOT / args.v0_metrics_path
    raw_prior_metrics = _load_raw_metrics(raw_metrics_file)
    v0_prior_metrics = _load_raw_metrics(v0_metrics_file)
    ...
    final_metrics, rich_final_buckets, final_predictions = evaluate_model_with_buckets(
        model, eval_loader, device, prediction_sample_limit=args.prediction_sample_limit
    )

    final_state = deepcopy(model.state_dict())
    model.load_state_dict(best_state)
    rich_best_metrics, rich_best_buckets, best_predictions = evaluate_model_with_buckets(
        model, eval_loader, device, prediction_sample_limit=args.prediction_sample_limit
    )
    ...
    v0_best_prior = v0_prior_metrics["raw_plus_v0_strategy_tags_model_best"]
    v0_final_prior = v0_prior_metrics["raw_plus_v0_strategy_tags_model_final"]
    bucket_comparison = _bucket_comparison(raw_final_buckets, rich_final_buckets)
    best_bucket_comparison = _bucket_comparison(raw_final_buckets, rich_best_buckets)
    metrics = {
        ...
        "raw_plus_v0_strategy_tags_model_final_prior_run": v0_final_prior,
        "raw_plus_v0_strategy_tags_model_best_prior_run": v0_best_prior,
        "raw_plus_rich_tags_many_signal_model_final": final_metrics,
        "raw_plus_rich_tags_many_signal_model_best": {"epoch": best_epoch, **rich_best_metrics},
        ...
        "deltas": _comparison_metrics(
            ...
            raw_best_prior,
            final_metrics,
            {"epoch": best_epoch, **rich_best_metrics},
            v0_best_prior,
            v0_final_prior,
            eq_metrics,
        ),
        "bucket_comparison_raw_final_vs_rich_final": bucket_comparison,
        "bucket_comparison_raw_final_vs_rich_best": best_bucket_comparison,
    }
    deltas = metrics["deltas"]
    wb.log_metric_groups(
        {
            "final/rich": final_metrics,
            "best/rich": {"epoch": float(best_epoch), **rich_best_metrics},
            "final/raw_checkpoint": raw_final_metrics,
            "prior/raw_best": raw_best_prior,
            "prior/v0_best": v0_best_prior,
            "prior/v0_final": v0_final_prior,
    ...
    wb.update_summary(
        {
            ...
            "best_mean_regret": rich_best_metrics["mean_regret"],
            "best_match_rate": rich_best_metrics["match_rate"],
            "best_tail_regret_rate_ge_5": rich_best_metrics["tail_regret_rate_ge_5"],
            "raw_best_mean_regret": raw_best_prior["mean_regret"],
            "v0_best_mean_regret": v0_best_prior["mean_regret"],
            "rich_best_minus_raw_best_mean_regret": deltas[
                "rich_best_minus_raw_best_mean_regret"
            ],
            "rich_best_minus_v0_best_mean_regret": deltas["rich_best_minus_v0_best_mean_regret"],
    ...
    manifest = {
        "schema_version": "w42.rich_tag_many_signal_probe.v1",
        "bead_id": "t42-csw6.12",
        ...
        "v0_baseline_artifacts": {
            "metrics_path": args.v0_metrics_path,
            "prior_run_metrics_loaded": True,
        },
        "generation": {
        ...
        "leakage_exclusions": [
            ...
            "v0 strategy tags and rich derived signals are public-state/action-local detector outputs",
        ...
        "tags_available": {
            ...
            "chapter_derived_tags": {
                "rich_global_signal_names": RICH_GLOBAL_SIGNAL_NAMES,
                "rich_action_signal_names": RICH_ACTION_SIGNAL_NAMES,
                "source": "public-state-safe aggregates and interactions derived from v0 tensors and v1 bucket map",
            },
    ...
    _write_jsonl(out_dir / "predictions_sample_rich_best.jsonl", best_predictions)
    _write_jsonl(out_dir / "predictions_sample_rich_final.jsonl", final_predictions)
    ...
    wb.log_artifact_files(
        name=f"w42-rich-tag-many-signal-{_git_sha()[:8]}",
        artifact_type="w42-probe",
        paths=[
            ...
            out_dir / "predictions_sample_rich_best.jsonl",
            out_dir / "predictions_sample_rich_final.jsonl",
