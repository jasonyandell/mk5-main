...
def manifest(root: Path, commit: str, source_mode: str, sources: list[Path], seed: int) -> dict[str, Any]:
    return {
        ...
        "source_corpora": [
            ...
            for path in (sources or [Path("w42/data_adapter_smoke/example_row.json")])
        ],
        "generation": {
            "command": "python w42/data_adapter_smoke.py --seed 42 --batch-size 1",
            "cwd": str(root),
            "environment": {
                "device": "cpu",
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="w42/data_adapter_smoke")
    ...
    report = {
        ...
        "example_row_path": display_path(out_dir / "example_row.json", root),
        "manifest_path": display_path(out_dir / "manifest.json", root),
    }
