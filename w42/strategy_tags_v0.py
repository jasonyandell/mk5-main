...
for _entry in (str(_ROOT_FOR_IMPORTS / "w42"), str(_ROOT_FOR_IMPORTS)):
...
def ensure_repo_imports(root: Path) -> None:
    root_str = str(root)
    w42_root = str(root / "w42")
    for entry in (w42_root, root_str):
...
def build_manifest(
    ...
    source_paths = sources or [root / "w42" / "data_adapter_smoke" / "example_row.json"]
    return {
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="w42/strategy_tags_v0")
    ...
    commit = git_sha(root)
    command = f"python w42/strategy_tags_v0.py --seed {args.seed} --limit {args.limit}"
