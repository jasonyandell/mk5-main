...
This is a w42-owned check suite for t42-csw6.9. It validates the existing
...
for entry in (str(ROOT / "w42"), str(ROOT)):
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="w42/detector_tests")
    args = parser.parse_args()

    torch.manual_seed(SEED)
    validate_tag_dims()
    checks = fixture_checks()
    detector_map = json.loads((ROOT / "w42/strategy_tags_v1_map/detector_map.json").read_text())
    map_rows, coverage = map_checks(detector_map)
    checks.extend(map_rows)

    command = f"python w42/detector_tests.py --out-dir {args.out_dir}"
