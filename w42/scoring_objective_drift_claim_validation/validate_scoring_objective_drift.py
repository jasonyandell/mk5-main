...
for entry in (REPO_ROOT / "w42", REPO_ROOT):
    text = str(entry)
...
OUT_DIR = Path("w42/scoring_objective_drift_claim_validation")
        ...
        entries.append(
            {
                ...
                "evidence_artifact": "w42/scoring_objective_drift_claim_validation/summary.json",
                "status": status,
                "caveats": caveats,
...
def main() -> None:
    ...
    wandb_run = init_wandb(
        ...
        tags=["w42", args.bead_id, "strategy-validation", "scoring", "eighty-four", "tournament", "promoted"],
    )
