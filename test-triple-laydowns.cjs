// Test for 3 simultaneous laydowns - much more feasible than 4
// Each of 3 players gets their trump double + 4 highest available trumps
// 4th player gets remaining dominoes
...
function generateTripleCombinations() {
  ...
  // Generate all combinations of 3 different trump suits
  for (let a = 0; a < trumpValues.length; a++) {
    for (let b = a + 1; b < trumpValues.length; b++) {
      for (let c = b + 1; c < trumpValues.length; c++) {
        combinations.push([trumpValues[a], trumpValues[b], trumpValues[c]]);
      }
    }
  }
...
function tryAllocateTripleHands(trumpCombination) {
  // Try to allocate 5 trumps to each of the 3 laydown players
  const hands = [[], [], [], []];
  const used = new Set();
  
  // Get trump dominoes for each of 3 players
  const playerTrumps = trumpCombination.map(trump => getTrumpDominoes(trump));
  
  // Allocate 5 trumps per laydown player (double + 4 others)
  ...
  // Distribute remaining dominoes (28 - 15 = 13 remaining)
  // Give 2 more to each laydown player (total 7 each)
  // Give 7 to the 4th player
  ...
  // Give 2 more dominoes to each of first 3 players
  for (let i = 0; i < 6; i++) {
    const player = Math.floor(i / 2);
    hands[player].push(remaining[i]);
  }
  
  // Give remaining 7 dominoes to 4th player
  for (let i = 6; i < 13; i++) {
    hands[3].push(remaining[i]);
  }
...
// Game simulation functions (same as before)
...
function testTripleLaydowns() {
  console.log("=".repeat(80));
  console.log("TESTING 3 SIMULTANEOUS LAYDOWNS");
  console.log("Each of 3 players gets their trump double + 4 highest available trumps");
  console.log("=".repeat(80));
  
  const allCombinations = generateTripleCombinations();
  console.log(`Total trump combinations to test: ${allCombinations.length}\n`);
  
  let candidatesGenerated = 0;
  let candidatesTested = 0;
  let validTripleLaydowns = 0;
  let totalIndividualTests = 0;
  let totalIndividualPassed = 0;
  
  for (const trumpCombo of allCombinations) {
    const hands = tryAllocateTripleHands(trumpCombo);
    ...
    // Show hands for first 3 players (the laydown candidates)
    for (let i = 0; i < 3; i++) {
      const trumpCount = trumpCombo[i] === 7 
        ? hands[i].filter(d => d[0] === d[1]).length
        : hands[i].filter(d => d[0] === trumpCombo[i] || d[1] === trumpCombo[i]).length;
      
      console.log(`  P${i + 1} (${trumpCombo[i] === 7 ? 'Doubles' : trumpCombo[i]}s, ${trumpCount}T): ${hands[i].map(d => `${d[0]}-${d[1]}`).join(', ')}`);
    }
    
    // Show 4th player's hand too
    console.log(`  P4 (mixed): ${hands[3].map(d => `${d[0]}-${d[1]}`).join(', ')}`);
    
    // Test each of the first 3 players as bidder (only they should have laydowns)
    const results = [];
    for (let biddingPlayer = 0; biddingPlayer < 3; biddingPlayer++) {
    ...
    console.log(`  Results: ${results.map((r, i) => `P${i+1}:${r ? '✅' : '❌'}`).join(' ')} (${passedCount}/3)`);
    
    if (passedCount === 3) {
      validTripleLaydowns++;
      console.log(`  🎉 VALID TRIPLE LAYDOWN FOUND! (#${validTripleLaydowns})`);
      
      // Show this in detail
      console.log(`  ${"=".repeat(50)}`);
      console.log(`  TRIPLE LAYDOWN DETAILS:`);
      for (let i = 0; i < 3; i++) {
        const trumpCount = trumpCombo[i] === 7 
          ? hands[i].filter(d => d[0] === d[1]).length
          : hands[i].filter(d => d[0] === trumpCombo[i] || d[1] === trumpCombo[i]).length;
        
        console.log(`    Player ${i + 1} (${trumpCombo[i] === 7 ? 'Doubles' : trumpCombo[i]}s trump, ${trumpCount} trumps):`);
        console.log(`      ${hands[i].map(d => `${d[0]}-${d[1]}`).join(', ')}`);
      }
      console.log(`    Player 4 (remaining dominoes):`);
      console.log(`      ${hands[3].map(d => `${d[0]}-${d[1]}`).join(', ')}`);
      console.log(`  ${"=".repeat(50)}`);
    }
    
    // Show progress occasionally
    if (candidatesGenerated % 10 === 0) {
      console.log(`\n--- Progress: ${candidatesGenerated} candidates tested, ${validTripleLaydowns} triple laydowns found ---\n`);
    }
  }
  
  console.log(`\n${"=".repeat(80)}`);
  console.log("TRIPLE LAYDOWN RESULTS:");
  ...
  console.log(`Valid triple laydowns: ${validTripleLaydowns}/${candidatesTested}`);
  
  if (validTripleLaydowns > 0) {
    console.log(`\n✅ CONCLUSION: ${validTripleLaydowns} confirmed triple laydown(s) found!`);
    console.log("3 simultaneous laydowns ARE possible in Texas 42.");
  } else {
    console.log(`\n❌ CONCLUSION: No valid triple laydowns found.`);
    console.log("Even 3 simultaneous laydowns appear to be impossible in Texas 42.");
  }
  console.log(`${"=".repeat(80)}`);
}
