...
#   ZEB_UPLOAD_INTERVAL     Seconds between HF uploads (default: auto)
#   ZEB_ORACLE_CHECKPOINT   Eval-aux oracle checkpoint
#   ZEB_EQ_N_SAMPLES        Eval-aux world samples per decision (default: 100)
#   ZEB_EQ_GAMES_PER_BATCH  Eval-aux games per batch (default: 128)
#   ZEB_EQ_BATCH_SIZE       Eval-aux --eval-batch-size (default: 0)
#   ZEB_EQ_TEMPERATURE      Eval-aux Zeb temperature (default: 0.1)
#   ZEB_EQ_POLICY_TARGETS   true|false (default false)
...
if [ "$WORKER_MODE" = "eval_aux" ]; then
    echo "  Oracle:   $ORACLE_CHECKPOINT"
    echo "  EQ cfg:   n_samples=$EQ_N_SAMPLES games_per_batch=$EQ_GAMES_PER_BATCH eval_batch_size=$EQ_BATCH_SIZE temp=$EQ_TEMPERATURE policy_targets=$EQ_POLICY_TARGETS"
fi
if [ -f "$BLACKLIST_FILE" ]; then
...
[ -n "$DRY_RUN" ] && echo "  Runtime:  ${YELLOW}DRY-RUN (no instances created/destroyed)${RESET}"
echo ""
