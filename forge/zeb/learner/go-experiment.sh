...
# go-experiment.sh - start learner for an experiment with persistent file logging.
#
# Defaults target the current lb-v-eq-3740 Stage 1 run.
# Override with env vars as needed.
#
# Example:
#   ZEB_WEIGHTS_NAME=lb-v-eq-3740 ./forge/zeb/learner/go-experiment.sh
...
WEIGHTS_NAME="${ZEB_WEIGHTS_NAME:-lb-v-eq-3740}"
RUN_NAME="${ZEB_RUN_NAME:-$WEIGHTS_NAME}"
WANDB_PROJECT="${ZEB_WANDB_PROJECT:-zeb-mcts}"
CHECKPOINT="${ZEB_CHECKPOINT:-forge/zeb/checkpoints/lb-v-eq-3740-bootstrap.pt}"
...
LOG_DIR="${ZEB_LOG_DIR:-history/t42-bzsk/logs}"
LOG_FILE="${ZEB_LEARNER_LOG_FILE:-$LOG_DIR/stage1-learner.log}"
PID_FILE="${ZEB_LEARNER_PID_FILE:-history/t42-bzsk/stage1-learner.pid}"
...
nohup "$PYTHON_BIN" -u -m forge.zeb.learner.run \
  ...
  --eval-aux-enabled \
  --eval-aux-batch-fraction "${ZEB_EVAL_AUX_BATCH_FRACTION:-0.05}" \
  --eval-aux-policy-weight "${ZEB_EVAL_AUX_POLICY_WEIGHT:-0.0}" \
  --eval-aux-value-weight "${ZEB_EVAL_AUX_VALUE_WEIGHT:-1.0}" \
  --eval-aux-belief-weight "${ZEB_EVAL_AUX_BELIEF_WEIGHT:-0.5}" \
