...
Orthogonal to the rungs: seat labels are a **rotation gauge** (Jason,
2026-07-22). Absolute seats 0..3 are arbitrary — simultaneous rotation of
viewer, bidder, leader, attribution and voids is value-invariant — but
bidder-relative offsets (`seat − bidder mod 4`) are physics: moving a void
from the left opponent to the partner is a different game. The quotient is
by the rotation only; the canonical form anchors on the winning bidder
(rotate bidder to seat 0, a 4× address merge). Certifiable as probe P-ROT
([#94](https://github.com/jasonyandell/mk5-main/issues/94)); together with
P-ATTR it pins the exact quotient — rotation free, relative structure
load-bearing, tile-level attribution drywall.
...
is scoped to tile relabelings (census gate R6), not seat symmetry; the
deliberate resolution is the bidder-anchored canonical form above
...
## The refinement reading (not an atlas)

The space is not a manifold and the object is not chart-covered — it is a
**refinement scheme**, p-adic in spirit (Jason, 2026-07-22). The record's
information events are digits: deal, bids, declaration, plays — each
appends, none retracts. The natural distance is the longest-common-prefix
**ultrametric**, under which balls are nested-or-disjoint: nothing glues,
everything nests (why "atlas" misnames the doctrine; the code artifact
keeps the name). Balls are cylinder sets — "known prefix, delimited tail";
the `(P, k)` cells are finite unions of them; the tower of quotients is
the inverse system, a coordinate at any rung a *truncation*, the full path
the limit element; belief is a measure built digit-by-digit on cylinders
(the per-step likelihood fold of [[belief-policy-value-algebra]]).

The two geometries split along the factorization: the ultrametric is the
intrinsic geometry of the **path factor** (evidence, belief, likelihood);
the congruence quotients path space onto the **physics DAG**, where prefix
distance degenerates — C4 pairs diverge at trick 3 (maximally far in the
tree) yet share one residue. Value lives on the quotient; belief lives on
the ultrametric. Scope of the analogy: topological and metric only —
branching is inhomogeneous (no single base p), depth is finite (28 plays),
and there is no ring structure; the algebra of 42 remains the suit
algebra.
