...
6. **Detach the trainer; resumable checkpoints; catch-all snapshot save.** Run-3's working attempt was killed at iter 487/1343 (val 0.302, healthy curve) by an MLX `RuntimeError: [metal::malloc] Resource limit (499000) exceeded` in the cosine LR scheduler. The `train_mlx` exception handler only catches the local `EarlyStopRequested`, so the in-memory best snapshot was lost. Future runs need: (a) `nohup setsid` launch so a parent-shell death doesn't take the trainer, (b) periodic on-disk adapter writes + `--resume-from` flag (current `steps_per_save=10**9` and end-of-run-only snapshot serialization mean every crash is total loss), (c) catch generic exceptions in `train_mlx` and serialize the in-memory best before re-raising. Postmortem: [[burl-star-run3]].
