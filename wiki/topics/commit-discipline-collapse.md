...
- **Surface-level fix:** bump rollout turn cap (8 → 12) to push the failure-mode threshold past where most natural commits happen. Doesn't *teach* discipline but recovers training-corpus yield. Cheap; harness-side. Adopted for harvest-2 (see [[burl-star-run3]]'s "What's next" §"FORCED_COMMIT inflation diagnosis (closed)").
