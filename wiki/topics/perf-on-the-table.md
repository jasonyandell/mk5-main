...
2. **Continuous batching — directionally confirmed at the bench layer ([[burl-perf-phase2]]).** Implemented at the bench layer as `run_bench_continuous` (`burl/eval/bench_decision_latency.py`) atop `mlx_lm.generate.BatchGenerator`. All decisions submit to a long-lived dispatcher; tools dispatch + state transitions happen on CPU as soon as a stream finishes, while other streams keep decoding. Initial Phase-2 wall numbers (71 s → 34–40 s, ~1.8–2.1×) were collected during scribe-B's parallel-job window **and** sit inside the bench's intrinsic 3.4× wall variance on `perf_subset_5` (memory `project_perf_subset_5_noise_floor`: 40–134 s same-config across sessions). The Phase-2 magnitude claim therefore needs either a back-to-back paired re-run in one quiet session **or** the 560-decision phase-exit pass before it counts. The structural argument for the win — that mlx-lm's `BatchGenerator` is already a continuous scheduler we just weren't using — does not depend on the wall measurement. Production harvest migration deferred; needs a cohort abstraction over [[batched-harvest-resilience]]'s wave-sentinel plumbing. Design at [[continuous-batching-dispatcher-design]].
