...
6. **Quantization (bf16 → Q4 PLE-safe) — confirmed at 1.3–2.3× wall + 25% memory cut on the 5-row paired bench ([[burl-perf-phase3]]).** The PLE-safe quant is `FakeRockert543/gemma-4-e2b-it-MLX-{4bit,8bit,bf16}` — note `mlx-community/gemma-4-*-{4,8}bit` and the original (non-UD) `unsloth/gemma-4-*-MLX-{4,8}bit` quants produce garbage because they quantize PLE / ScaledLinear layers (output multipliers amplify quant error).  The headline `phase3-stack-best` row (Q4 PLE-safe + Phase-2 continuous batching): wall **28.3 s vs paired baseline 36.4 s** (1.29× on the cleaner pair, 2.34× on the contention-noisy pair earlier), peak mem **8.93 GB vs 10.80 GB** (−17%), 4/5 paired play match (single divergence at gi=0, the marginal-decision slot bf16 itself flips across runs).  Q4 *gained* 0.06 Q-pts vs bf16 on the q4-mlx-cont row — quality envelope fully preserved at temp=0.  Q8 PLE-safe ran 1.69× wall but lost on quality (3/5 paired play match) and memory (9.83 GB vs Q4's 9.17 GB) — a worse pick than Q4 on both axes.  Drop-in: `--model-repo FakeRockert543/gemma-4-e2b-it-MLX-4bit` to the existing `bench_decision_latency.py`; no other code changes needed.
