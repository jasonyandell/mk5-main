...
- [[experiments/w42-champion-selfplay-fixed-point|w42-champion-selfplay-fixed-point]] — Champion rung #26: the belief-conditioned bidder (hypothetical-completed-auction + belief-weighted oracle E[Q]) closes the self-play loop, which **converges to a stable fixed point** (belief-KL 0.116→~0.08 plateau, 0.045 seed floor) — but a **stable over-bidder** (loses to net:wp ~−3.4 marks/game; double-dummy P(make) > achievable PIMC). An optimism correction (pmake_scale=0.70) halves the loss + doubles wins, confirming the diagnosis, yet net:wp stays stronger — belief value is legibility, not marks. Champion exported to plunge as the `onyx` player (complete)
