...
- [[experiments/burl-perf-phase3|burl-perf-phase3]] — Phase 3 spec (research-mode pause): two dragons forced the bench plan to bend — mlx-lm 0.31.2 spec-decode is single-stream-only (does not stack on Phase 2's `BatchGenerator`), and there is no Gemma 4 E0.5B (smallest Gemma 4 is E2B itself); plan covers PLE-safe Q4/Q8 quants + Gemma-3-270M-as-draft tokenizer-compat probe + spec-stream-self fallback; numbers TBD post-RESUME (spec)
