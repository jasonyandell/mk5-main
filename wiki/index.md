...
- [[experiments/burl-perf-phase3|burl-perf-phase3]] — Phase 3 result: speculative-decoding lever ruled out (mlx-lm spec-decode is single-stream-only; Gemma-3-270M's tokenizer collapses Gemma 4 special tokens to byte-fallback so acceptance ≈ 0 on tool-call regions); Q4 PLE-safe quant (`FakeRockert543/gemma-4-e2b-it-MLX-4bit`) ships clean — `phase3-stack-best` 28.3 s wall, 8.93 GB peak, 4/5 paired play match, *gains* 0.06 Q-pts vs bf16 (active)
