...
- [[topics/mlx-cohort-bench-discipline|mlx-cohort-bench-discipline]] — Apple Silicon unified-memory GPU is one resource; parallel scribes contend invisibly at the process layer; serialize benches and use decode_tok_s as the contention detector (active)
...
- [[experiments/burl-perf-phase1|burl-perf-phase1]] — Phase 1 cheap wins: per-prompt max_tokens plumbed end-to-end via mlx-lm 0.31.2's batch_generate(List[int]); per-state cap lands flat-2048 because tighter caps flip clean-commits to forced-commits; Lever 2 (parallel tool dispatch) not implemented — wax-museum gate produces 1 call/turn at sub-ms execution; 5-row noise floor too noisy to attribute lever wall deltas (active)
