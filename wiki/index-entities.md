...
- [[entities/hoyt|hoyt]] — the game's own referee: net-free kernel (BR re-solves 0.9 ms, 45×) + verified CFR+ references (gap ≤0.05 pts in ≤40 iters, scale-invariant in worlds) + exploitability meter + frozen 200-root eval anchor; prices strategies against the game, not against our nets; "according to Hoyle," computed (active)
- [[entities/metal-hoyt|metal_hoyt]] — the GPU searcher under the fp64 referee: hoyt's CFR+ iterate + steering gap as raw MSL kernels on the M5 Max (searcher 18.2× paired, wedge iterate 34×), fp32 by license, every banked claim certified by hoyt's exact fp64 BR; `refsweep --engine metal` (active)
