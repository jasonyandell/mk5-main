...
- **Q:** Is the per-root H4 reference VALUE well-defined under the partnership seam — and how wide is the basin spread across the 200-root evalset?
  - Raised: metal_hoyt line cross-check 2026-07-21 ([[perf-log]] 21a; [[metal-hoyt]])
  - Context: seed 555013 — two honestly-converged CFR references (single-seat gap ~0.03, identical 199 worlds, bit-identical walt-BR value) differ by **1.59 pts** in self-play value. Single-seat BR gap does not bound value differences in a two-team game (hoyt's registered honesty line, now with a measured witness). The frozen bank is bit-stable because the fp64 engine is deterministic — one trajectory, one basin. Open: measure basin multiplicity per root (e.g. seed the fp64 engine from several starts), decide whether the stable eval should quote value ± basin spread, and whether team-pair (TMECor-style) pricing is worth building.
