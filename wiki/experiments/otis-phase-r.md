...
| R4 | Posterior weights recorded | every decision in the weight-recording files (v2 train + v2 eval, fixed sampling) carries per-world posterior weights, sum = 1 ± 1e-5; ESS distribution reported descriptively (no band — first measurement). *Amended pre-run, before any regen started:* the adaptive-path files (v1 train/eval) do not record weights — past-step tokenization at adaptive world counts is a VRAM hazard and their consumers (Q-training, the Bayes ceiling) do not read weights; adaptive-path recording is filed as follow-up work | weights missing or unnormalized on any decision in a weight-recording file | |
