...
- Output directory contains all six listed artifacts plus a README not listed above; summary.json and the CSVs corroborate every headline number on this page.
- Cheap next probe: the `supported-for-generated-trace-proxy` taxonomy violation in the scoring worker's summary.json was never cleaned up — a one-line fix plus ledger note would close that loop.
- The ch03-laydown-correctness fixture-substrate split remains pending; worth checking whether a later wave absorbed it.
