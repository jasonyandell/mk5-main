...
- `scratch/jud-v0/loop/run_loop.py` exists only in the main checkout (scratch/ is
  gitignored); mirroring it into `champion/evidence/jud_v0/` (or `forge/`) would let the
  loop's driver survive scratch cleanup.
- The predicted-exceedance MAE claims (0.020 vs 0.118) trace to `step2b_report.md` and were
  not independently recomputed; a cheap probe is a script re-deriving both from
  `margin_net_eval.json` + `optimism_gap.json`.
