...
- Headline numbers verified against `scratch/champion-run/run24_RESULTS.txt` on the main checkout; the file is gitignored, so they are not reproducible from the repo alone.
- Cheap next probe: rerun the arena A/B with a non-oracle play policy (net:wp or jud) — the page's thesis predicts the belief gain shows marks value only when play is imperfect, turning the interpretation from inference into measurement.
