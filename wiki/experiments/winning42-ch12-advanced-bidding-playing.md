...
| Count pounces on bidder offs are mandatory against high bids even when the defender is unsure who wins. | context-limited (powered at bid >= 39) | [[w42-bookval-v1-wave2-bid-aware-atlas]] paired Q-delta evidence: bid=39 setter Q delta `+0.376` (CI `[+0.010, +0.751]`), bid=42 `+0.410` (CI `[+0.068, +0.765]`) — both CIs exclude zero in book direction. The pounce signal flips between bid=30 (Wave 2.E showed pounce wrong-by-EV) and bid >= 39 (book direction). Snapshot-level evidence still pending via Wave 2.E.2 (bead `t42-8kbh`). Ledger row promoted from `underpowered` to `context-limited`. |
