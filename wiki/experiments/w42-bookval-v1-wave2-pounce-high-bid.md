...
- Atlas `.pt` files (`bid_aware_atlas/eq_pdf_seeds*_bid*_v2.pt`) are not in the repo — only manifest/CSVs — so the "n_samples=200 per decision" claim traces to the manifest, not the tensors.
- The flagged "setter team led" follow-up (39.6% pounce better, N=280) never ran; a cheap next probe filters to setter-partner-led tricks with count currently on the table.
