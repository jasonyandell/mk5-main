...
Two-pass audit against code and artifacts; 2 corrections applied in place and independently re-verified; second pass amended 3 (percentage-point glosses had a factor-2 error: mark utility spans ±1, so Δutility = 2·Δp(win)).

- The run's own README (`w42/book_validation_v1/wave1/t42-c6sa_mark_utility_transform/README.md`) still carries the wrong 10/55/84.6%/0.044 numbers and a "~4.4 percentage points" gloss with the same factor-2 error; a one-line errata is warranted in a session touching that directory. Wiki numbers trace to `action_mark_ev_scalars.csv` and `detector_endorsed_flips.csv`, not the README.
- The 12-vs-10 gap is exactly the two genuine flips with no named book-position detector (game 6 decision 13, gain 0.030; game 8 decision 10, gain 0.002 — `matched_position_detectors` is nan, only shape tags fire). Cheap probe: inspect those two decisions to see whether the detector vocabulary is missing a family or they are noise (genuine gains overall span 0.002–0.092).
