...
status: superseded
superseded_by: burl-iter1-adapter
---
...
- **Loss masking**: full sequence including user turn (same LEM default — could flip to
  `assistant_only_loss=True` for iter-1 per [[sft-completion-only-loss]])
...
2. **Primer is too long.** ~40K chars/decision of rules text displaced attention from
   decision-making. See [[primer-tradeoff]].
...
[[burl-iter1-adapter]], which re-harvested the corpus on a trimmed-primer prompt targeting
recovery toward spike v2's 88.9%. Iter-0 demonstrated a real signal: training on the corpus
is possible and clean; the problem is what's in the corpus.
