...
## [2026-07-21 | worktree-metal-hoyt | metal_hoyt — the GPU searcher under the fp64 referee]

**Touched pages:** [[entities/metal-hoyt|metal-hoyt]] [[entities/hoyt|hoyt]] [[topics/perf-log|perf-log]] [[index-entities]] [[questions/open|open]]
**Added:** [[entities/metal-hoyt|metal-hoyt]] — new engine (not a port): hoyt's CFR+ iterate + steering gap as raw MSL kernels (mx.fast.metal_kernel, zero new deps) on the M5 Max GPU, fp32 by measured license, every banked claim certified by hoyt's fp64 exact BR. Searcher 18.2× (16-root paired bench), wedge iterate 34×, whole-solve 1.39× (Amdahl moved to CPU build/certify/export). `refsweep --engine metal`: 200/200 converged, 960 s vs CPU 990, h2 post-threads-fix 420 s (fastest half measured), ~half the CPU per eval.
**Verdicts:** fold kernels not bitwise vs numpy (Metal FMA contraction; 1.8e-7 vs fp64 same-order fold, repeatable) — M2 gate is ulp+determinism; fp32 steering drift worst 8.5e-3 (argmax flips), margin 0.01; 19d oversubscription trap re-fired in the metal path, fixed.
**Finding:** 555013 — two honestly-converged references 1.59 pts apart at gaps ~0.03 (identical worlds, bit-identical walt BR): the partnership seam measured at scale; single-seat gap does not pin value. Question filed in [[questions/open|open]]. Follow-ups: #87 (CPU levers: build/certify/batching), #88 (H5/H6 habitat).
